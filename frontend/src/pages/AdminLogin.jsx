...
import { Lock as LockIcon, CheckCircle as CheckCircleIcon, Error as ErrorIcon } from '@mui/icons-material';
...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await apiClient.postJson('/api/admin/login', { email, password });
      if (result?.token) {
        // Store under the same keys for simplicity so rest of app can access
    ...
    <Box className="login-container" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '70vh', p: 2 }}>
      <Paper elevation={0} sx={{ p: 4, maxWidth: 420, width: '100%' }}>
        <Box sx={{ textAlign: 'center', mb: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary">Sign in to manage CarvoHub</Typography>
        </Box>
