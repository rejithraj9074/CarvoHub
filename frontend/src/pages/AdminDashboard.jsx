...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        const [overview, bookingsRes, mechanicsRes, usersRes, carwashRes] = await Promise.all([
          ...
          apiClient.getJson('/api/carwash?limit=50')
        ]);
        ...
        <Box sx={{ p: { xs: 2, md: 4 }, flex: 1 }}>
            ...
            <Paper elevation={0} sx={{ p: 3 }}>
              ...
              <Drawer anchor="right" open={mechFormOpen} onClose={() => setMechFormOpen(false)}>
                <Box sx={{ width: { xs: 360, md: 480 }, p: 3 }} role="presentation">
                  <Typography variant="h6" fontWeight={700} sx={{ mb: 2 }}>{mechForm.id ? 'Edit Mechanic' : 'Add Mechanic'}</Typography>
                  <Grid container spacing={2}>
          ...
          {activeSection === 'carwash_mgmt' && (
            <Paper elevation={0} sx={{ p: 3 }}>
              ...
              <Divider sx={{ mb: 2 }} />
              <Grid container spacing={2}>
                {(bookings || []).filter(b => b.serviceType || b.location).map((b) => (
                  <Grid key={b._id} item xs={12} md={6}>
                    <Paper elevation={0} sx={{ p: 2 }}>
                      <Typography variant="subtitle1" fontWeight={700}>{b.serviceType || 'Car Wash'}</Typography>
                      <Typography variant="body2" color="text.secondary">Customer: {b.customer?.name || '—'}</Typography>
                      <Typography variant="body2" color="text.secondary">Date: {new Date(b.scheduledDate || b.date).toLocaleDateString()} {b.scheduledTime || b.timeSlot}</Typography>
                      <Typography variant="body2" color="text.secondary">Location: {b.location?.type || b.location}</Typography>
                      <Box sx={{ display: 'flex', gap: 1, mt: 1, flexWrap: 'wrap' }}>
                        <Button size="small" variant="outlined" color="success" onClick={async ()=>{
                          try {
                            if (b.serviceType && b.timeSlot !== undefined) {
                              await apiClient.putJson(`/api/carwash/${b._id}/status`, { status: 'Confirmed' });
                            } else {
                              await apiClient.putJson(`/api/bookings/${b._id}/status`, { status: 'confirmed' });
                            }
                            const [bookingsRes, carwashRes] = await Promise.all([
                              apiClient.getJson('/api/bookings?limit=50'),
                              apiClient.getJson('/api/carwash?limit=50')
                            ]);
                            const combined = [
                        ...
                        <Button size="small" variant="outlined" color="error" onClick={async ()=>{
                          try {
                            if (b.serviceType && b.timeSlot !== undefined) {
                              ...
                              await apiClient.putJson(`/api/bookings/${b._id}/status`, { status: 'cancelled' });
                            }
                            const [bookingsRes, carwashRes] = await Promise.all([
                              apiClient.getJson('/api/bookings?limit=50'),
                              apiClient.getJson('/api/carwash?limit=50')
                            ...
                            setBookings(combined);
                            setToast({ open: true, type: 'success', message: 'Rejected' });
                          } catch (er) {
                            setToast({ open: true, type: 'error', message: er?.message || 'Reject failed' });
                          }
                        }}>Reject</Button>
                      </Box>
                    </Paper>
