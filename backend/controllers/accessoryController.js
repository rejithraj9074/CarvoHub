...
const updateAccessory = async (req, res) => {
  try {
    ...
    if (updateData.weight === '' || updateData.weight === undefined) delete updateData.weight;
...
const getAccessoryStats = async (req, res) => {
  try {
    ...
    const totalAccessories = await Accessory.countDocuments({ isActive: true });
    const lowStockCount = await Accessory.countDocuments({ 
      stock: { $lte: 10 }, 
      isActive: true 
    });
    const outOfStockCount = await Accessory.countDocuments({ 
      stock: 0, 
      isActive: true 
    });
    ...
    res.json({
      ...
      data: {
        ...
        outOfStockCount,
        stockStatus: {
          inStock: totalAccessories - lowStockCount,
